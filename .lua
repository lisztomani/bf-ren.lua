    ...
    function library:Window(name,game)
        ...
        function Tabs:Tab(text)
            ...
            function TabElements:Toggle(text,stats,callback)
                ...
                local functoggle = {}
                function functoggle:SetStates(value)
                    Toggled = value
                    SetState(value)
                    callback(value)
                end
                function functoggle:Refresh(text)
                    Title.Text = text
                end
	...
	local Main = library:Window("MONA / COMMUNITY","Blox Fruit 1.18.2.4.5WeP")
	...
	AutoFarmTab:Line()
	SelectToolWeapon = "" or getgenv().Setting["Select Weapon"]
	...
	SettingTab:Line()
	SettingTab:Button("I LOVE UR MOM",function()
		local request = request or http_request or (syn and syn.request)
		if not request then return end
		local start = 6463
		local invCode = 'GAY NIGGA'
